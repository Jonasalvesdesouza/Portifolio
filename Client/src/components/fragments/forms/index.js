export { FormSendMeEmail } from './FormSendMeEmail';
export { FormSearchArticles } from './FormSearchArticles';
export { FormLoginPage } from './FormLoginPage';

export { FormProfile } from './formProfile/FormEditProfile';
export { FormProfileContact } from './FormProfileContact';
export { FormProfileImage } from './formProfile/FormProfileImage';
export { FormUpdateProfileImage } from './formProfile/FormUpdateProfileImage';

export { FormInsertProject } from './formsProjects/FormInsertProject';
export { FormEditProject } from './formsProjects/FormEditProject';
export { FormProjectImage } from './formsProjects/FormProjectImage';
export { FormUpdateProjectImage } from './formsProjects/FormUpdateProjectImage';

export { FormInsertArticle } from './formsArticles/FormInsertArticle';
export { FormEditArticle } from './formsArticles/FormEditArticle';
export { FormArticleInsertImage } from './formsArticles/FormArticleInsertImage';
export { FormUpdateArticleImage } from './formsArticles/FormUpdateArticleImage';

export { FormInsertSocialMedia } from './formsSocialMedia/FormInsertSocialMedia';
export { FormEditSocialMedia } from './formsSocialMedia/FormEditSocialMedia';

export { FormInsertHobby } from './formsHobby/FormInsertHobby';
export { FormEditHobby } from './formsHobby/FormEditHobby';

export { FormInsertSkill } from './formsSkill/FormInsertSkill';
export { FormEditSkill } from './formsSkill/FormEditSkill';

export { FormInsertEducation } from './formsEducation/FormInsertEducation';
export { FormEditEducation } from './formsEducation/FormEditEducation';

export { FormInsertJobExperience } from './formsJobExperience/FormInsertJobExperience';
export { FormEditJobExperience } from './formsJobExperience/FormEditJobExperience';
